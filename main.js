    ...
    function specificFoodInfo(ID) {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                nutrients.append(`<tr class="nutrients-row"><td class="nutrients-cell">${data.foodNutrients[1].nutrient.name}s</td><td class="nutrients-cell nutrients-cell-amount">${parseFloat(data.foodNutrients[1].amount).toFixed(2)}</td><td class="nutrients-cell nutrients-cell-unit">${data.foodNutrients[1].nutrient.unitName}</td></tr>`);
            ...
            if (!input.replace(/\s/g, '').length) {   
                ...
                $.ajax({                
                    ...
                    success: function(data) {
                        ...
                        for(let i = 0; i < 10; i++) {
                            listElement = $(`<li tabIndex="${i}" class="result" value="${i}">${data.foods[i].description}</li>`);
                            listElement.hide();
                            resultsList.append(listElement);
                            listElement.delay(`${i}00`).fadeIn(`${i}00`);
                        }
